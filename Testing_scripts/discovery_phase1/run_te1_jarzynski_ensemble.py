#!/usr/bin/env python3
"""Run TE₁ Jarzynski rediscovery ensemble and summarize metrics."""
...
DEFAULT_OUTPUT = DEFAULT_PROJECT_ROOT / "runs" / "discovery_phase1" / "te1_jarzynski_ensemble.csv"
DEFAULT_STEPS = 6000
DEFAULT_RUNS = 10
DEFAULT_WORKERS = 9
...
def run_single(seed: int, scenario: str, scenario_dir: Path, steps: int, workers: int) -> Dict[str, Any]:
    ...
    report_path = Path(run_dir) / "report.yaml"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase I TE₁ Jarzynski rediscovery ensemble runner")
    parser.add_argument("--scenario", default="discovery_phase1/te1_jarzynski.yaml")
    ...
    for seed in range(1, args.runs + 1):
        print(f"[TE1-Ensemble] Seed {seed}/{args.runs}")
    ...
    print(f"TE₁ ensemble summary written to {output_path}")
